...
def workfor(t):
    """fake working for t second, CPU activity will raise
    :param t: working duration i s
    :type t: float
    """
    start = time.time()
...
class NoResult(object):
    """
    object that will be returned by a process if it fails
    """
    pass
...
class Job(object):
    """
    an object to pack arguments and keyword arguments
    with a syntax similar to a function call
    """
...
class MainThreadInterrupt(Exception):
    """the error to raise if the main thread is the cause of the process interruption"""
    pass


class WorkerInterrupt(Exception):
    """the error to raise if one worker is the cause of the process interruption"""
    pass


class Worker(Process):
    """
    object that will execute operation in a separate workspace
    methods __call__ and cancel must be customized by the user
    """
    def __init__(self, job, lock=None, seed=None, ppid=None, verbose=False):
        """
        :param job:
        :param lock:
        :param seed:
    ...
    def rand(self, n=1):
        """
        generate random numbers using the seed attached to the worker
        :param n:
        :return:
        """
        ...
        for worker in self.workers:
            if worker.is_alive():
                if worker.interruption_signal_sent.value:
                    ...
                    if self.verbose:
                        print("main thread  do not send the interruption signal to ",
                              worker.name, worker.pid, "(interruption was coming from it)")
                    continue
                if self.verbose:
                    print("main thread  send the interruption signal to ", worker.name, worker.pid)
...
if __name__ == '__main__':
    ...
    job_list = [Job(*np.random.randn(2)) for _ in range(8)]
    try:
